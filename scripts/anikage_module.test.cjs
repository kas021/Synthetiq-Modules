...
const entry = catalogue.modules.find(item => /^modules\/Anikage-/.test(item.file));
...
function load(fetcher) {
  ...
  vm.runInContext(source.replace('// ---------- Exports on globalThis ----------',
    'globalThis.helpers = { subtitleEntries, marker, mediaUrl, validateStreamPlayback, requestJson, retryAfterMs };'), context);
  return context;
}
...
test('429 stops provider cascade and subsequent calls during Retry-After', async () => {
  let calls = 0;
  const context = load(async () => {
    calls++;
    return { status: 429, headers: { 'Retry-After': '120' } };
  });
  await assert.rejects(context.extractStreamUrl('title:ep:1', 'sub'), /rate limited/);
  await assert.rejects(context.extractStreamUrl('title:ep:1', 'dub'), /rate limited/);
  assert.equal(calls, 1);
});
test('cooldowns are host scoped and expire without scheduling background work', async () => {
  let now = 100000, calls = 0, timers = 0;
  const context = load(async () => {
    calls++;
    return calls === 1 ? { status: 429, headers: { 'retry-after': '2' } } : response({ ok: true });
  });
  vm.runInContext('Date.now = () => testNow();', Object.assign(context, { testNow: () => now }));
  context.setTimeout = () => { timers++; };
  await assert.rejects(context.helpers.requestJson('https://anikage.cc/a'), /rate limited/);
  await context.helpers.requestJson('https://other.test/a');
  await assert.rejects(context.helpers.requestJson('https://anikage.cc/b'), /rate limited/);
  now += 2001;
  await context.helpers.requestJson('https://anikage.cc/b');
  assert.equal(calls, 3);
  assert.equal(timers, 0);
});
test('Retry-After supports HTTP date, header accessor and missing header', () => {
  const { retryAfterMs } = load().helpers;
  assert.equal(retryAfterMs({ headers: { get: () => '90' } }), 90000);
  assert.equal(retryAfterMs({ headers: {} }), 60000);
  const delay = retryAfterMs({ headers: { 'retry-after': new Date(Date.now() + 90000).toUTCString() } });
  assert.ok(delay > 88000 && delay <= 90000);
});
